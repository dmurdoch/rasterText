...
SEXP get_strings(Glyph_atlas& atlas) {
  ...
  for (int i=0; i < atlas.strings.size(); i++) {
    String_record& s = atlas.strings[i];
    SET_STRING_ELT(VECTOR_ELT(result, 0), i, mkChar(s.text.c_str()));
...
extern "C" {
  ...
  void build_atlas(Glyph_atlas& atlas, const char * text, const char* family, int font, float size,
                   int col);
  ...
  SEXP build_atlasR(SEXP text, SEXP family, SEXP font, SEXP cex, SEXP rgb, SEXP monochrome, SEXP prevatlas){
    ...
    for (int i=0; i < Rf_length(text); i++) {
      ...
      build_atlas(atlas, CHAR(STRING_ELT(text, i)),
...
void build_atlas(Glyph_atlas& atlas,
                 const char* text,
                 const char* family,
                 int rfont,
